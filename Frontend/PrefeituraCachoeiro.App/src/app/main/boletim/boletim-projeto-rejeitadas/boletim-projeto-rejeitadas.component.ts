...
@Component({
  selector: 'app-boletim-projeto-rejeitadas',
  templateUrl: './boletim-projeto-rejeitadas.component.html',
  styleUrls: ['./boletim-projeto-rejeitadas.component.scss']
})
export class BoletimProjetoRejeitadasComponent implements OnInit {
  ...
  async carregarBoletinsMedicao(medicaoId: number) {
     ...
     this.listaMedicoesSelecionadasAgrupados.medicaoResponse.forEach(async element => {
       ...
       await this._boletimControllerService.BuscarBoletiRecusado(boletimDetalhadoRequest)
       .then(async (dados) => {
         if (dados.detalhes.length != 0) {
           this.boletim = dados;
           //Se tornou boletim geral
